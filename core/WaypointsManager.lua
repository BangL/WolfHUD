...
if string.lower(RequiredScript) == "lib/setups/setup" then
		...
		for component_name, component_data in pairs(data) do
			if type(component_data) == "table" and component_data.type then
				if not self:add_component(component_name, data, nil) then
					WolfgangHUD:print_log(string.format("Error while creating waypoint: %s", self._id), "error")
				end
			end
		end
	...
	function CustomWaypoint:_setup_text_component(name, data, base_size, base_scale, base_panel)
		...
		self._components[name] = base_panel:text({
			...
			color = color,
			font = component_data.font or data.text_font or tweak_data.gui:get_font_path(tweak_data.gui.fonts.din_compressed, font_size),
			font_size = font_size,
			align = "left",
			vertical = "center",
			w = size * 3,
