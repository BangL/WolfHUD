if RequiredScript == "lib/units/enemies/cop/copdamage" then
	...
	function CopDamage:_process_popup_damage(data)
		CopDamage.DMG_POPUP_SETTING = JimHUD:getSetting({"DamagePopup", "DISPLAY_MODE"}, 3)
	...
	function CopDamage:show_popup(damage, dead, headshot, critical, color_id)
		if managers.waypoints then
			...
			local waypoint_color = color_id and ((color_id == 5 and JimHUD:getSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "USE"}, true)) and JimHUD:getColorSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "COLOR"}, "white") or tweak_data.chat_colors[color_id]) or JimHUD:getColorSetting({"DamagePopup", critical and "CRITICAL_COLOR" or headshot and "HEADSHOT_COLOR" or "COLOR"}, "yellow")
			waypoint_color = waypoint_color:with_alpha(JimHUD:getSetting({"DamagePopup", "ALPHA"}, 0.8))
			local waypoint_duration = JimHUD:getSetting({"DamagePopup", "DURATION"}, 2)
			if waypoint and not waypoint:is_deleted() then
				...
				local params = {
					unit = self._unit,
					offset = Vector3(10, 10, JimHUD:getSetting({"DamagePopup", "HEIGHT"}, 30)),
					scale = 2 * JimHUD:getSetting({"DamagePopup", "SCALE"}, 0.6),
					...
					icon = {
						type = "icon",
						show = dead,
						scale = JimHUD:getSetting({"DamagePopup", "SKULL_SCALE"}, 0.9),
					...
					component_order = { JimHUD:getSetting({"DamagePopup", "SKULL_ALIGN"}, 2) == 1 and { "icon", "label" } or { "label", "icon" } , { "duration" } }
				}
