if not JimHUD:getSetting({"DamageIndicator", "ENABLED"}, true) then return end
...
if string.lower(RequiredScript) == "lib/managers/hud/hudhitdirection" then
	...
	function HUDHitDirection:_add_hit_indicator(...)
		HUDHitDirection.PANEL_SIZE = JimHUD:getSetting({"DamageIndicator", "SIZE"}, 150)
		if self.indicator_count < JimHUD:getSetting({"DamageIndicator", "MAX_AMOUNT"}, 10) then
	...
	function HUDHitDirection:_animate(indicator, data, remove_func)
		data.duration = JimHUD:getSetting({"DamageIndicator", "DURATION"}, 2)
		data.t = 0
	...
	function HUDHitDirection:_get_indicator_color(damage_type, t)
		...
		elseif damage_type == HUDHitDirection.DAMAGE_TYPES.CRIT then
			return JimHUD:getColorSetting({"DamageIndicator", "CRIT_COLOR"}, "purple")
		elseif damage_type == HUDHitDirection.DAMAGE_TYPES.FRIENDLY_FIRE then
			return JimHUD:getColorSetting({"DamageIndicator", "FRIENDLY_FIRE_COLOR"}, "orange")
		else
			return Color(1, t, t)
		end
	end
